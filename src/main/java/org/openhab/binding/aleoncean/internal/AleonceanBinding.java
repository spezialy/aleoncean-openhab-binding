...
public class AleonceanBinding extends AbstractBinding<AleonceanBindingProvider> implements ManagedService {
    ...
    protected void internalReceiveCommand(String itemName, Command command) {
        ...
        final WorkerReply reply = worker.addAndWaitForReply(new WorkerItemReceivedCommand(itemName, command), 1, TimeUnit.MINUTES);
        if (reply.getReplyCode() != WorkerReplyCode.OK) {
    ...
    protected void internalReceiveUpdate(String itemName, State newState) {
        ...
        final WorkerReply reply = worker.addAndWaitForReply(new WorkerItemReceivedState(itemName, newState), 1, TimeUnit.MINUTES);
        if (reply.getReplyCode() != WorkerReplyCode.OK) {
            LOGGER.warn("Something went wrong ({}) on internal received command.", reply.getReplyCode());
        }
    }
    ...
    public void updated(Dictionary<String, ?> config) throws ConfigurationException {
        if (config != null) {
            ...
            LOGGER.warn("You need to set at least the port for ESP3 hardware if you want to use the aleoncean binding.");
        }
    }
